...
for p = 1:m:numel(all_files)
    ...
    % regression tree (see https://uk.mathworks.com/help/stats/regressiontree-class.html)
    tree = fitrtree(oct_training, force_training);
    force_prediction = predict(tree, oct_validation);

    % model evaluation (see https://en.wikipedia.org/wiki/Regression_validation)
    mse = [mse, immse(force_prediction, force_validation)];
